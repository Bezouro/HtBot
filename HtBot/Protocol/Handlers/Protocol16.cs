...
namespace MinecraftClient.Protocol.Handlers
{
    ...
    class Protocol16Handler : IMinecraftCom
    {
        ...
        private static OnChat Chat = new OnChat();
        private byte[] keepAlive = new byte[5] { 0, 0, 0, 0, 0 };
        public static Protocol16Handler instance = null;
        private bool kalive;
        private int kaliveCount;
        Thread threadAntiAFK;
        ...
        public void StartThreadAntiAFK()
        {

            if (threadAntiAFK == null)
            {
                threadAntiAFK = new Thread(RunnableAntiAFK);
            }

            if (!threadAntiAFK.IsAlive) {
                threadAntiAFK.Start();
            }
        }

        private void RunnableAntiAFK()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    switch (rdm) {
                        case 1: SendChatMessage("/home"); break;
                        case 2: SendChatMessage("/g &6");  break;
                        case 3: SendChatMessage("HtBot :)");  break;
                        case 4: SendChatMessage("/spawn");  break;
                        case 5: SendChatMessage("/warp loja");  break;
                    }

                    if (kaliveCount == 0)
                    {
                        //SendChatMessage("/ativar ReconectandohtBot");
                        //Thread.Sleep(3000);
                        //Program.Restart();
                    }

                    ConsoleIO.WriteLineFormatted("§e" + kaliveCount + " §4Updates!");
                    kaliveCount = 0;
                    //ConsoleIO.WriteLineFormatted("§e[AntiAFK] §7Mantendo a conta online [" + rdm + "] :D");
                    Thread.Sleep(10000);
                    
                }
                catch (Exception)
                {
                    ConsoleIO.WriteLineFormatted("§e[AntiAFK] §7Finalizando anti-Afk");
                    threadAntiAFK.Abort();
                }
            }
        }
        ...
        {
            //ConsoleIO.WriteLine("[Packet] Entrada de pacote " + id + "Size: ");
            int nbr = 0;
            switch (id)
            {
                ...
                case 0x03:
                    ...
                    Chat.onchat(message, false);
                    StartThreadAntiAFK(); break;
                ...
                default:
                    ConsoleIO.WriteLineFormatted("Erro: Pacote desconhecido " + id.ToString() + "\n" + readNextByte());
                    //SendChatMessage("/ativar ReiniciandoHtbot");
        ...
        {
            //ConsoleIO.WriteLineFormatted("§6[Packet Send] Enviando pacote!");
            if (encrypted)
            {
        ...
        public static bool doPing(string host, int port, ref int protocolversion)
        {
            try
            {
                ...
                if (ping[0] == 0xff)
                {
                    ...
                    ConsoleIO.WriteLineFormatted("§3[Conexão]§9 Servidor " + version + " (protocolo v" + protocolversion + ").");
                    return true;
                }
                else return false;
            }
