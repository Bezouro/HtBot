    ...
    public static class ProtocolHandler
    {
        ...
        public static bool MinecraftServiceLookup(ref string domain, ref ushort port)
        {
            if (!String.IsNullOrEmpty(domain) && domain.Any(c => char.IsLetter(c)))
            {
                try
                {
                    ConsoleIO.WriteLineFormatted("§3[Conexão] §6Verificando servidor [" + domain + "]");
                    ...
                    if (srvRecords != null && srvRecords.Any())
                    {
                        ...
                        ConsoleIO.WriteLineFormatted(String.Format("§3[Conexão] §6conectando ao servidor {0}:{1}", target, result.PORT, domain));
                        domain = target;
                        port = result.PORT;
                        return true;
                    }
        ...
        private static int DoHTTPSRequest(List<string> headers, string host, ref string result)
        {
            ...
            AutoTimeout.Perform(() =>
            {
                ...
                {
                    ConsoleIO.WriteLineFormatted("ProtocolHandler:497");
