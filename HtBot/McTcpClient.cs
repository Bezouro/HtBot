...
namespace MinecraftClient
{
    ...
    public class McTcpClient : IMinecraftComHandler
    {
                ...
                try
                {
                    if (handler.Login())
                    {
                        ...
                        else
                        {
                            ...
                            ConsoleIO.WriteLineFormatted("§3[Conexão] §2Conectado ao servidor!");
        ...
        public bool PerformInternalCommand(string command, ref string response_msg)
        {
            ...
            {
                Telegram.SendHtmlMessage("[Console] " + Command.getArg(command));
            }
        ...
        public void OnTextReceived(string text, bool isJson)
        {
            ...
            foreach (ChatBot bot in bots.ToArray())
            {
                ...
                {
                    ConsoleIO.WriteLineFormatted("McTcpClient:464");
        ...
        public void OnConnectionLost(ChatBot.DisconnectReason reason, string message)
        {
            ...
            switch (reason)
            {
                case ChatBot.DisconnectReason.ConnectionLost:
                    message = "§3[Conexão]§c Desconectado do servidor: Conexão perdida!";
                    ConsoleIO.WriteLineFormatted(message);
                    break;

                case ChatBot.DisconnectReason.InGameKick:
                    ConsoleIO.WriteLineFormatted("§3[Conexão]§c Desconectado pelo servidor: " + message);
                    break;

                case ChatBot.DisconnectReason.LoginRejected:
                    ConsoleIO.WriteLineFormatted("§3[Conexão]§c Erro ao conectar: " + message);
                    if (message.StartsWith("[Sistema Anti DC]")) {
                        Program.Restart(7);
                    }
                    break;
            }
        ...
        public void OnUpdate()
        {
            foreach (var bot in bots.ToArray())
            {
                ...
                {
                    ConsoleIO.WriteLineFormatted("McTcpClient:518");
                    if (!(e is ThreadAbortException))
                    {
